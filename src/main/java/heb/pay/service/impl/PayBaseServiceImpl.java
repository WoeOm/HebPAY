...
public class PayBaseServiceImpl implements PayBaseService {
	...
	public Map<String,Object> checkBaseInfo(String payKey, String payerTypecode, double amt) {
		...
		//验证银行信息
		List<PayWay> payWayList = payCenterService.getPayWayList(payKey);
	...
	public Map<String, Object> checkOrderExist(String orderNo, String userNo,
		...
		List<PaymentOrder> orderList = payCenterService.getPaymentOrderByOrderNum(orderNo,userNo,payerTypeCode,busiTypeCode,amt);
		Map<String, Object> map = new HashMap<String, Object>();
	...
	public Map<String, Object> checkBankData(LinkedHashMap<String, String> params,String bankName) {
		...
		if(bankName.equals("ccb")){
			...
			List<MerchantPayInfo> merPayInfoList = payCenterService.getMerPayInfoByPostid(postId, bankId);
			//执收单位不存在
		...
		List<PaymentOrder> payOrderList = payCenterService.getPayOrderByOrderId(orderNo);
		//订单不存在
		if(payOrderList == null || payOrderList.size()<=0){
			map.put("error","no_order");
			return map;
		}
		List<PayerType> payerTypeList = payCenterService.getPayerType(payOrderList.get(0).getProduct_name());
		//支付类型不存在
		...
		//支付类型
		List<PayWay> payWayList = payCenterService.getPayWayByCode(payOrderList.get(0).getPay_way_code(),payOrderList.get(0).getPay_type_code());
		//商户支付配置信息
		List<MerchantPayConfig> merPayConfigList = payCenterService.getMerPayConfig(payOrderList.get(0).getMerchant_no(),payWayList.get(0).getPay_product_code());
		//返回数据
	...
	public Map<String, Object> insertZFOrderRecord(String payWayCode,
		...
		PayWay payWay = payCenterService.getPayWayByCode(payWayCode,payTpyeCode).get(0);
	...
	public int insertPayOrder(PaymentOrder paymentOrder,String payerName) {
		//插入支付平台表
		int ret = payCenterService.insertPaymentOrder(paymentOrder);
		if(ret>0){
			//获取单位信息
	...
	public int updateOrderStatus(String orderId, String statusDB) {
		int ret = payCenterService.updateOrderByOrderId(orderId,statusDB);
		if(ret>0){
			ret = payCenterService.updateRecordByOrderId(orderId,statusDB);
		}
		return ret;
	}
	...
	public Map<String,Object> getPayOrderByOrderId(String orderId) {
		List<PaymentOrder> payOrderList = payCenterService.getPayOrderByOrderId(orderId);
		Map<String,Object> map = new HashMap<String, Object>();
		if(payOrderList == null || payOrderList.size()<=0){
			map.put("error",-1);
			return map;
		...
		String notifyUrl = payCenterService.getNotifyUrlByOrderId(orderId);
